 ...
 * entirely in JavaScript. Only calls C# for Escape (close) and submenu/menu switching.
 *
 * Modes:
 ...
 *   "submenu"  — Submenu content (same as vertical + ArrowLeft to close submenu)
 */
 ...
 * @param {object} config
 * @param {string} config.mode - "vertical" (default), "menubar", or "submenu"
    ...
    switch (e.key) {
      ...
      case 'ArrowRight':
        if (mode === 'menubar') {
          ...
          // For vertical/submenu modes: if focused item has aria-haspopup, click to open submenu
          const focused = document.activeElement;
          if (focused && focused.getAttribute('aria-haspopup') === 'menu' && container.contains(focused)) {
            e.preventDefault();
            focused.click();
          }
        }
      ...
      case 'ArrowLeft':
        if (mode === 'menubar') {
          ...
          e.preventDefault();
          dotNetRef.invokeMethodAsync('JsOnCloseSubMenu').catch(() => {});
        }
        break;
    }
  };
